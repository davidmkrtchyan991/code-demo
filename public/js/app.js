...
$(document).ready(function () {
    ...
    $(".add-more").click(function (e) {
        ...
        next = next + 1;
        var newIn = '<input autocomplete="off" class="input form-control" id="field' + next + '" name="prop[]" type="text">';
        var newInput = $(newIn);
    ...
    $(".add-more-exceptional").click(function (e) {
        ...
        var addRemove = "#field" + next;
        next = next + 1;
        var newIn = '<input autocomplete="off" class="input form-control" id="field' + next + '" name="exceptional-items[]" type="text">';
        var newInput = $(newIn);
        var removeBtn = '<button id="remove' + (next - 1) + '" class="btn btn-danger remove-me" >-</button></div><div id="field">';
        var removeButton = $(removeBtn);
        $(addto).after(newInput);
    ...
    var userPassFields = '<div class="form-group{{ $errors->has(\'password\') ? \' has-error\' : \'\' }}">\n' + '            <label for="password" class="col-md-4 control-label">Пароль</label>\n' + '\n' + '            <div class="col-md-6">\n' + '                <input id="password" type="password" class="form-control" name="password">\n' + '\n' + '            </div>\n' + '        </div>\n' + '\n' + '        <div class="form-group">\n' + '            <label for="password-confirm" class="col-md-4 control-label">Повторите пароль</label>\n' + '\n' + '            <div class="col-md-6">\n' + '                <input id="password-confirm" type="password" class="form-control" name="password_confirmation">\n' + '            </div>\n' + '        </div>';

    var changeUserPassBlock = $(".changeUserPassFieldsBlock");
    var changeUserPassBtn = $(".changeUserPassFieldsBtn");
    ...
    function getOrderPositions() {
        if ($("#order").attr('data-order-domain')) {
            var currentDomain = $("#order").data('order-domain');
            var currentDomainId = $("#order").data('order-domain-id');
            $(".api-spinner").show();
            $.ajax({
                type: "POST",
                url: '/getapi',
    ...
    function fillInPositionsForOrder(data) {
        ...
        var summaryKeys = Object.keys(data.positions_summary.tops[1]);
        var summaryDynamicsKeys = Object.keys(data.positions_summary.tops_dynamics);
        var first = data.positions_summary.tops[1][summaryKeys[0]];
        var second = data.positions_summary.tops_dynamics[summaryDynamicsKeys[0]];
        var positionSummaryAvg = data.positions_summary.avgs[1];

        var positionSummaryDynamicsAll = data.positions_summary.dynamics.all;
        var positionSummaryDynamicsDown = data.positions_summary.dynamics.down;
        var positionSummaryDynamicsStay = data.positions_summary.dynamics.stay;
        var positionSummaryDynamicsUp = data.positions_summary.dynamics.up;

        var positionSummaryTops1to3 = data.positions_summary.tops[1][summaryKeys[0]];
        var positionSummaryTops1to10 = data.positions_summary.tops[1][summaryKeys[1]];
        var positionSummaryTops11to30 = data.positions_summary.tops[1][summaryKeys[2]];
        var positionSummaryTops31to50 = data.positions_summary.tops[1][summaryKeys[3]];
        var positionSummaryTops51to100 = data.positions_summary.tops[1][summaryKeys[4]];
        var positionSummaryTops101to10000 = data.positions_summary.tops[1][summaryKeys[5]];

        var positionSummaryDynamicsTops1to3 = data.positions_summary.tops_dynamics[summaryKeys[0]];
        var positionSummaryDynamicsTops1to10 = data.positions_summary.tops_dynamics[summaryKeys[1]];
        var positionSummaryDynamicsTops11to30 = data.positions_summary.tops_dynamics[summaryKeys[2]];
        var positionSummaryDynamicsTops31to50 = data.positions_summary.tops_dynamics[summaryKeys[3]];
        var positionSummaryDynamicsTops51to100 = data.positions_summary.tops_dynamics[summaryKeys[4]];
        var positionSummaryDynamicsTops101to10000 = data.positions_summary.tops_dynamics[summaryKeys[5]];
...
window.validations = {
    ...
    initOrderFormValidations: function initOrderFormValidations() {
        var $orderForm = $("#order-form");
        var _self = this;
        $orderForm.validate({
            invalidHandler: function invalidHandler(e, validator) {
                if (validator.errorList.length) {
                    var closestTabId = jQuery(validator.errorList[0].element).closest(".tab-pane").attr('id');
    ...
    validateTab: function validateTab(element) {
...
window.datePickersManager = {
    ...
    initDatepickerIfExists: function initDatepickerIfExists($element) {
        if ($element.length > 0) {
...
window.optimizerFinder = {
    ...
    optimizerFinderCallback: function optimizerFinderCallback(userData) {
        var userAsJSON = $.parseJSON(userData);
        $("#optimizers-suggesstion-box").hide();
...
window.userFinder = {
    ...
    userFinderCallback: function userFinderCallback(userData) {
        var userAsJson = $.parseJSON(userData);
        this.updateUserInfo(userAsJson);
        this.resetSearchForm();
    },

    updateUserInfo: function updateUserInfo(userAsJson) {
    ...
    resetSearchForm: function resetSearchForm(resetEmailToFind) {
...
window.faqCategoryFinder = {
    ...
    faqCategoryFinderCallback: function faqCategoryFinderCallback(faqCategoryData) {
        var faqCategoryAsJson = $.parseJSON(faqCategoryData);
        this.updateFaqCategoryInfo(faqCategoryAsJson);
        this.resetSearchForm();
    },

    updateFaqCategoryInfo: function updateFaqCategoryInfo(faqCategoryAsJson) {
        $("#faqCategoryToFind").val(faqCategoryAsJson.category);
    },

    resetFaqCategoryInfo: function resetFaqCategoryInfo() {},
...
var currentEmail = $('#firechat-wrapper-container').data('user-email') || null;
var currentUserName = $('#firechat-wrapper-container').data('user-name') || null;
var currentUserRole = $('#firechat-wrapper-container').data('user-role') || null;
        ...
        firebase.auth().onAuthStateChanged(function (user) {
            if (user) {
                if (currentUserRole === 'ROLE_ADMINISTRATOR' || currentUserRole === 'ROLE_TECHNICAL_MANAGER' || currentUserRole === 'ROLE_OPTIMIZER') {
                    var newEmail = currentEmail.split('.').join("");
                    user.updateProfile({
...
var newUserName = "";
var userNewAvatar = $(".user-new-avatar");
...
function initChat(user) {
    ...
    setTimeout(function () {
        chat.on('message-add', function () {
            ...
            var audio = new Audio('../../sounds/open-ended.ogg');
            var messagesCountNumber = 0;
            audio.play();
...
function chatToggle() {
    var firechatWrapperContainer = $("#firechat-wrapper-container");
    var firechatHeaderMinimize = $("#firechat-header-minimize");
    var glyphiconArrow = $(".firechat-header-minimize .glyphicon");
